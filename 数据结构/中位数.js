...
function findMidNum(nums1, nums2) {
    let m = nums1.length
    let n = nums2.length
    let k1 = Math.floor((m + n + 1) / 2)
    let k2 = Math.floor((m + n + 2) / 2)
    const value1 = findMedianSortedArraysCore(nums1, 0, nums2, 0, k1)
    const value2 = findMedianSortedArraysCore(nums1, 0, nums2, 0, k2)
...
const nums1 = [1,4,5,7,10,18], nums2 = [2,3,4,9,100]
const mid = findMidNum(nums1, nums2);
console.log(mid);
